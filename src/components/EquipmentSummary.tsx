 ...
 * see AutomaticGearImport's confirmItem), falling back to the generic mascot icon otherwise. Fire
...
function CategoryThumb({ category, entry }: { category: EquipmentCategory; entry?: EquipmentEntry }) {
  const photo = entry?.photoDataUrls?.[0] ?? entry?.extinguisherUnits?.[0]?.photoDataUrls?.[0];
  if (!photo) {
