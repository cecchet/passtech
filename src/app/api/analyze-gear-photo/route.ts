...
const CATEGORY_HINTS: Record<(typeof CLASSIFIABLE_CATEGORIES)[number], string> = {
  ...
  fire_suppression: "an onboard fire suppression SYSTEM — a fixed cylinder plumbed to nozzles around the engine/cockpit, mounted in the car (not a handheld extinguisher you'd carry). This category also covers that system's own external pull cable/handle/pin (often on the cowl, dash, or exterior bodywork, sometimes red, often next to a system-brand decal like Lifeline/SPA/OMP/Safecraft) used to trigger it manually -- that pull cable is part of the fire suppression system, NOT a kill switch, even though both can look like 'a red pull mechanism on the car.'",
  kill_switch: "a master electrical kill switch / battery cutoff — a rotary or pull switch, often red, mounted on the car's exterior or dash, that cuts all electrical power to the car. Don't confuse with a fire suppression system's external pull cable/pin (see fire_suppression) -- a kill switch is specifically electrical (often labeled with a lightning bolt or 'MASTER'/'BATT' symbol), not a fire-system trigger.",
