 ...
 * balaclava all together, no per-category scoping needed) was originally generated by the
 * generic pdftotext-based row parser and shipped as "verified," but spot checks found the same
 * row-drift bug as every older-era list here (DC.052.19-T, DC.077.20-G, DC.087.20-G all had
 * wrong or missing model/dates). Rebuilt from a user-provided CSV export of the source PDF
 * (fia-xls/list-74.csv) instead -- see LIST_CONFIG[74]'s comment in scripts/parse-fia-list.mjs
 * for the couple of source typos fixed on import (a transposed Gloves suffix letter on a handful
 * of numbers, one mistyped Shoes suffix, and the Overalls suffix digit-zero normalized to letter
 * "O" to match this list's own existing convention).
 *
