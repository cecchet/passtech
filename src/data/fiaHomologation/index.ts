 ...
 *
 * List 74 (protective clothing, FIA 8856-2018 -- firesuit/gloves/shoes/socks/undergarment/
 * balaclava all together under one list, unlike List 27/29's older-era split) was originally
 * generated by the generic pdftotext-based row parser and shipped as "verified," but spot checks
 * found the same row-drift bug as every older-era list here (DC.052.19-T, DC.077.20-G,
 * DC.087.20-G all had wrong or missing model/dates). Rebuilt from a user-provided CSV export of
 * the source PDF (fia-xls/list-74.csv) instead -- see LIST_CONFIG[74]'s comment in
 * scripts/parse-fia-list.mjs for the couple of source typos fixed on import (a transposed Gloves
 * suffix letter on a handful of numbers, one mistyped Shoes suffix, and the Overalls suffix
 * digit-zero normalized to letter "O" to match this list's own existing convention). This list is
 * also what motivated adding `FiaHomologationEntry.category` (see that field's doc comment in
 * src/data/types.ts): its CSV's PRODUCT column reliably says which of the 6 categories each entry
 * actually is, and without recording that per entry, a firesuit's homologation number typed into
 * the gloves field would have spuriously validated (this list covers both categories, and nothing
 * else was checking that the ENTRY, not just the LIST, actually matches). Derived automatically
 * at parse time via PRODUCT_TYPE_TO_CATEGORY, so any future CSV/spreadsheet-sourced list with a
 * real per-product category column gets the same protection for free.
 *
